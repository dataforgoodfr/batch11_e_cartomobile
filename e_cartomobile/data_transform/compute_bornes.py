...
def compute_bornes_by_communes(df_irve_power_clean):
    ...
    df_bornes_communes = df_cluster_plot.reset_index().pivot(index='code_commune_INSEE',columns='cluster')
...
def complete_df_irve(df_irve):
    ...
    df_irve[["x_crs_2154","y_crs_2154"]] = np.array(get_x_y_from_lat_lon(
                                df_irve.consolidated_latitude,
                                df_irve.consolidated_longitude)
                                ).T
...
def compute_bornes_by_communes_ponderated(gamma = 5, dist_max_km = 20):
    ...
    gdf_comm[["x_crs_2154","y_crs_2154"]] = np.array(get_x_y_from_lat_lon(gdf_comm.y,gdf_comm.x)).T
    df_bornes_communes_completed = df_bornes_communes.join(gdf_comm[["insee","x_crs_2154","y_crs_2154"]].set_index("insee"))

    # smooth 
    df_bornes_communes_smooth = add_close_pdc_by_power_cluster(df_bornes_communes_completed, df_irve, gamma, dist_max_km)

    return df_bornes_communes_smooth

def compute_bornes_by_communes_ponderated(df_bornes_communes_smooth, scenario = 'default'):

    return compute_bornes_ponderated(df_bornes_communes_smooth, get_scenario_weight(scenario))
