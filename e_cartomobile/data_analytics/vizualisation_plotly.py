...
def graph_station_types(irve_file_path="e_cartomobile/data_extract/data_for_viz/consolidation-etalab-irve-clean.csv"):
    """
    Graphique plotly montrant la répartition du type d'implantation des stations
    """
    # read the IRVE file
    irve = pd.read_csv(irve_file_path)
    # Create the graph (plotly express)
    fig = px.pie(irve['implantation_station'].value_counts().reset_index(),
                ...
                #hover_data={
                #        'index': False,
                #        'implantation_station': True,
                #        }
                )
    return fig

# Graph of the different connector types and the number of stations available
def graph_connector_types(irve_file_path="e_cartomobile/data_extract/data_for_viz/consolidation-etalab-irve-clean.csv"):
    """
    ...
    # Create the graph (plotly express)
    fig = px.bar(type_prise, y='Type', x='Nombre de points de charge', orientation='h', text_auto='.5',
        title='Types de connecteurs')
    return fig

# Graphs depicting the evolution of the number of charging stations
def graph_stations_evolution(irve_ts_file_path='e_cartomobile/data_extract/data_for_viz/irve_time_series.csv'):
    """
    2 graphiques plotly:
    ...
    """
    # Read the file
    irve_ts = pd.read_csv(irve_ts_file_path, index_col=0).set_index('date_mise_en_service')
    # Keep only data after 2015
    irve_ts_2015_now = irve_ts[irve_ts.index.values>'2015']
    # Create the 1st graph (stations) (plotly express)
    fig1 = px.line(irve_ts_2015_now.reset_index(), markers=True,
              x='date_mise_en_service', y='nb_stations',
              title="Evolution du nombre de stations",
              labels={'date_mise_en_service': 'Date',
                      'nb_stations': 'Nombre de stations'})
    # Create the 2nd graph (charging points) (plotly express)
    fig2 = px.line(irve_ts_2015_now.reset_index(), markers=True,
              x='date_mise_en_service', y='nb_pdc',
              title="Evolution du nombre de points de charge",
              labels={'date_mise_en_service': 'Date',
                      'nb_pdc': 'Nombre de points de charge'})
    fig2.update_traces(line_color='crimson')
    return fig1, fig2

# Graph of charging stations per electric vehicle per french departments
def graph_pdc_par_ve(pdc_immat_file_path="e_cartomobile/data_extract/data_for_viz/pdc_immat_par_dep.csv"):
    """
    ...
    # Create the graph (plotly express)
    fig = px.bar(pdc_immat_par_dep.sort_values(by='pdc_par_vp_el', ascending=True),
                x="pdc_par_vp_el", y="nom_departement", orientation='h', text_auto='.2', height=2000,
                title="Points de charge par voiture à recharge électrique",
                labels={'nom_departement': 'Département',
